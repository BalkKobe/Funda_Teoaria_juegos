...
void MainGame::handleInput()
{
	...
	if (inputManager.isKeyPressed(SDLK_f)) {
		cout << "Total de humanos: " << totalHumanos << endl;
		cout << "Total de zombies: " << totalZombies << endl;
	}
}
...
void MainGame::initLevel() {
	...
	const int numHumans = levels[currentLevel]->getNumHumans();
	const int numZombies = numHumans;
...
void MainGame::drawHud()
{
	...
	spriteFont->draw(hudBatch, buffer, glm::vec2(0, 0), glm::vec2(0.5), 0.0f, color);

	spriteFont->draw(hudBatch, buffer, glm::vec2(0, 64), glm::vec2(0.5), 0.0f, color);
